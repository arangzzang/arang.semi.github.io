...
<%
	//게시물 수정 jsp view
	int no=Integer.parseInt(request.getParameter("no"));//수정버튼에서 넘겨줘야하는 게시글 고유 번호
%>

<section class="contents-wrap">
	<h2>글 수정하기</h2>

	<form action="<%=request.getContextPath()%>/view/custom/modifyC.jsp"
		...
		<div class="cbox">
			...
			<div class="cbox_textbox">
				...
				<div class="c_titlebox content_row">
					<input type="text" id="ctitle" name="ctitle" class="form-control" placeholder="제목 입력">
				</div>
				<!-- 본문작성 div -->
				<div class="c_textbox content_row">
					<textarea name="ctext" class="form-control text-area_input" placeholder="내용을 입력하세요."></textarea>
				</div>
			</div>
		</div>
		...
		<!-- 글올리기 버튼 -->
		<div>
			<input type="submit" class="form-control_input_btn" value="글 수정">
		</div>
		<input type="hidden" value="<%=loginMember.getMemberId()%>"
			name="userId">
			<input type="hidden" value="<%=no%>" name="no1" id="no1">
	</form>
</section>
