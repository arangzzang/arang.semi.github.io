	...
	<div class="swiper-border">
		<div class="infromation-parent">
			...
			<div class="information">
				...
				<div>
					<img class="img-style"
						src="<%=request.getContextPath()%><%=p.getProductThumbnail()%>"
						alt="">
				</div>
				<!-- 상품 정보 -->
				<div class="productinformation">
						...
						<div class="su-form">
							<form class="count" name="form" method="get">
								<div id="price">
									...
									%>
									<p>
										<strong class="price-strong">판매가격</strong>
										<%=p.getProductPrice()%>원
									</p>
									<%
										}
								...
								<div class="product-cout">
									<strong>수량</strong> <input type="text" id="amount" value="1"
										size="3" name="su">
								...
								<strong class="money">총 상품금액</strong> <label id=sum></label>원
								<p class="product-delivery">
									※ 평일 오전 11시 이전에 주문 결제 완료하시면 다음날 받아 보실 수 있습니다. <br>
									&nbsp;&nbsp;&nbsp;단, 하루 제작량이 정해져 있어 주문량이 많은 날에는 배송 지연이 발생할 수 <br>
									&nbsp;&nbsp;&nbsp;있습니다. 이점 양해 부탁드리며, 배송 메세지에 희망 수령일을 기재해주시면 <br>
									&nbsp;&nbsp;&nbsp;최대한 반영해 드리고 있습니다.
								</p>
							</form>
						</div>
					...
					<div class="btn">
						<hr>
						<button class="jangbtn" onclick="movecart()">
							<p class="jangbtn-movecart">장바구니 담기</p>
						</button>
						<button class="orderbtn" onclick="moveorder()">
							<p class="jangbtn-movecart">주문하기</p>
						</button>
					</div>
				</div>
			</div>
				...
				<div class="swiper-wrapper a-wrapper">
					...
					<div class="swiper-slide a-slide">
						<ul>
							...
							%>
							<li><a class=" pridse"
								href="<%=request.getContextPath()%>/product/detailProduct?productNo=<%=list.get(j).getProductNo()%>">
									<img alt="" src="<%=list.get(j).getProductThumbnail()%>"
									class="product-img">

									<p class="prids"><%=list.get(j).getProductName()%></p>
									<p class="prids"><%=list.get(j).getSalePer() != 0
							? formatter.format(list.get(j).getProductPrice()
									- (list.get(j).getProductPrice() * list.get(j).getSalePer() / 100))
							: formatter.format(list.get(j).getProductPrice())%>원
									</p> <%
 	System.out.println(list.get(j).getSalePer());
 			System.out.println(formatter.format(list.get(j).getProductPrice()
 					- (list.get(j).getProductPrice() * list.get(j).getSalePer() / 100)));
 %>

							</a>
							<!-- p.getProductPrice()-(p.getProductPrice()*p.getSalePer()/100) -->
							</li>
							<%
								if (list.size() - 1 == j) {
											break;
		...
		<div id="infobtns">
			<div id="infobtns-ch">
				<ul id="infobtns-ch22">
					<li class="infobtns-ch3"><a class="infos-ch2"
						onclick="move('1')"> <span class="infos-ch33">상품정보</span> <span></span>
					</a></li>
					<li class="infobtns-ch3"><a class="infos-ch2"
						onclick="move('2')"> <span class="infos-ch33">후기보기</span> <span></span>
					</a></li>
					<li class="infobtns-ch3"><a class="infos-ch2"
						onclick="move('3')"> <span class="infos-ch33">배송안내</span> <span></span>
					</a></li>
				</ul>
				<!--     		<div class="infos-ch2"> 
			...
			<div id="review-go">
				<div id="infobtn2">
					<!-- 이동 2 -->
					<span id="icon"> <i class="far fa-comments fa-2x"></i>
					</span> <span id="font-color">지금, 후기를 작성해보세요!</span>
		...
		<span id="scroll"></span>
		<h1 id="views-list">등록된 리뷰</h1>
		<p id="tatal-Data"><%=totalData%></p>
		<%
				...
				<div id="puoduct_list_content" class="puoduct_list_content2">
					...
					<div class="comment-container">
						<div class="comment-editor">
							<form
								action="<%=request.getContextPath()%>/product/productReview"
					...
					<div class="review-item">
						<div class="wirter-color"><%=pc.getProductCommentWriter()%>
						</div>
						<div>
							<p><%=pc.getProductCommentContent()%></p>
						</div>
